...
class _HuggingFaceEncodingAdapter:
    """Expose the small tiktoken-like API used by the shard builders."""
    ...
    def __init__(self, tokenizer: HuggingFaceTokenizer, eos_token: str):
        ...
        if eos_id is None:
            raise ValueError(f"EOS token is absent from tokenizer vocabulary: {eos_token!r}")
...
def load_encoding(tokenizer_root: Path):
    ...
    return _HuggingFaceEncodingAdapter(tokenizer, eos_token), config
