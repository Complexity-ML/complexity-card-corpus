...
_CONCEPTS = (
    ...
    ("physics", "mass versus weight", "mass measures matter while weight is the force of gravity on that mass", "an object keeps its mass on the Moon but weighs less there", "changing gravity alters weight without changing mass"),
    ...
    ("biology", "natural selection", "heritable differences that affect reproduction change how common traits become", "better-camouflaged insects may leave more offspring", "the population changes across generations rather than one individual adapting by choice"),
    ("biology", "a vaccine", "it trains immune recognition before exposure to the disease", "immune memory can respond faster when the pathogen appears later", "protection comes from a prepared response, not from the vaccine causing the target illness"),
    ("computing", "a cache", "it keeps recently useful data closer to where it is needed", "a browser can reuse an image instead of downloading it again", "removing stale cached data can reveal a newer source version"),
    ("computing", "encryption", "it transforms readable data into a form that requires a key to interpret", "an encrypted file remains unintelligible without the decryption key", "encoding format alone is not encryption because it provides no secret"),
    ...
    ("networks", "DNS", "it maps human-readable names to network addresses", "a resolver can find the address associated with a website name", "a name-resolution failure can occur even when the destination server is running"),
    ...
    ("research", "a control group", "it provides a comparison for changes that might have happened without the tested intervention", "two similar groups can differ mainly in whether they receive the treatment", "the comparison helps separate treatment effects from background change"),
    ...
    ("civics", "separation of powers", "government authority is divided so institutions can limit one another", "a legislature makes laws while courts can review their application", "no single branch should control every core function unchecked"),
)
_AUDIENCES = (
