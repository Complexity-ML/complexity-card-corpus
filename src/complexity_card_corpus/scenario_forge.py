...
def _semantic_combinations(
    ...
    *,
    ...
    constraints = _permuted(family.constraints, f"{prefix}:constraint")
    states = _permuted(family.states, f"{prefix}:state")
    outcomes = _permuted(family.outcomes, f"{prefix}:outcome")
    combinations = list(itertools.product(intents, constraints, states, outcomes))
    return sorted(
        combinations,
    ...
    for family in registry.families:
        ...
        for domain in family.domains:
            ...
            for rank, (intent, constraint, state, outcome) in enumerate(
                ...
                scenario_id = f"scenario:{hashlib.sha256(signature.encode()).hexdigest()[:24]}"
                fallback = family.fallbacks[
                    _stable_index(
                        f"{registry.seed}:{signature}:fallback", len(family.fallbacks)
                    )
                ]
